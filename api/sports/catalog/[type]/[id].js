...
function metaFor(ch, host) {
  const dynamicPoster = `https://${host}/sports/poster/${ch.id}.png`;
  const poster = ch.game ? dynamicPoster : posterFor(ch);
  let name = ch.name;
  ...
  if (ch.game) {
    ...
    const title = isEventMode ? g.eventShortName || g.eventName || ch.name : `${g.awayTeam} @ ${g.homeTeam}`;

    name = title;
    if (g.status === 'in') {
      name += isEventMode ? ` — ${g.statusDetail || 'Live'}` : ` — ${g.awayScore}-${g.homeScore} (${g.statusDetail})`;
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ metas: filtered.map((c) => metaFor(c, host)) });
  } catch (err) {
