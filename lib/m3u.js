...
async function getChannels() {
  ...
  try {
    const compressed = await redis.get(CACHE_KEY);
    if (compressed) {
    ...
    // Redis unavailable or corrupt entry — fall through to a direct fetch below
  }
...
async function refreshM3uCache(opts = {}) {
  ...
  const payload = { channels: finalChannels, generatedAt: Date.now() };
  const compressed = zlib.gzipSync(Buffer.from(JSON.stringify(payload))).toString('base64');
  await redis.set(CACHE_KEY, compressed, { ex: CACHE_TTL_SECONDS });
