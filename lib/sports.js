...
async function fetchEspnScoreboard(path, dateStr) {
  try {
...
async function fetchPathAcrossDays(path) {
  const days = [dateStrOffset(-1), dateStrOffset(0), dateStrOffset(1)];
  const results = await Promise.all(days.map((d) => fetchEspnScoreboard(path, d)));
  const merged = [];
...
async function fetchLeagueSchedule(league) {
  ...
  if (league === 'tennis') {
    const [atpJson, wtaJson] = await Promise.all([
      fetchPathAcrossDays('tennis/atp'),
      fetchPathAcrossDays('tennis/wta')
    ]);
    ...
    if (!path) return [];
    const json = await fetchPathAcrossDays(path);
...
function matchVersusChannel(channel, games) {
  ...
  for (const g of games) {
    const direct = (wordOverlapRatio(teams.a, g.homeTeam) + wordOverlapRatio(teams.b, g.awayTeam)) / 2;
    const swapped = (wordOverlapRatio(teams.a, g.awayTeam) + wordOverlapRatio(teams.b, g.homeTeam)) / 2;
...
function matchEventChannel(channel, games) {
  ...
  for (const g of games) {
    const score = Math.max(
      wordOverlapRatio(g.eventName, cleaned),
      wordOverlapRatio(g.eventShortName, cleaned)
    );
    if (score > bestScore) {
      bestScore = score;
      best = g;
